...
from mariImageBrowser import cfg, core, utils, MIB_STYLESHEET

#------------------------------------------------#
# File Bookmark Widget
#------------------------------------------------#
class MIBConfigUI(QtWidgets.QDialog):
    '''MIBView Configuration Widget
    '''
    def __init__(self, parent=None):
        super(MIBConfigUI, self).__init__(parent)
    ...
    def buildConfigSetting(self, name, settingDict):
        ...
        if settingType == 'path':
            valueWidget = QtWidgets.QLineEdit(current)
            getValueFunc = valueWidget.text
...
class MIBBookmarkList(QtWidgets.QListWidget):
    ...
    def getPathsFromMime(self, mimeData):
        ...
        if mimeData.hasUrls():
            ...
            for url in mimeData.urls():
                path = QtCore.QDir.toNativeSeparators(url.toLocalFile())
                pathList.append(path)
        # Mari Paths
        mariImagePathFormat = 'Mari/MriImagePath'
        if mimeData.hasFormat(mariImagePathFormat):
            mriPathBA = mimeData.data(mariImagePathFormat)
            pathList = [ QtCore.QDir.toNativeSeparators(path) for path in mriPathBA.data().split('|') ]
        return pathList
    ...
    def renamePreset(self):
        ...
        newName, ok = inputDialog.getText(self,"MIBView","Rename Preset:", text=presetName)
        if newName and ok:
            try:
...
# File Browser Widget
#------------------------------------------------#
class MIBFileBrowser(QtWidgets.QWidget):
    '''MIBView Custom File Browser Widget
    '''
...
# Image List Widget
#------------------------------------------------#
class MIBImageList(QtWidgets.QListWidget):
    '''MIBView Custom QListWidget for thumbnail layout
    '''
    #_____________________________________________
    def __init__(self):
        super(MIBImageList, self).__init__()
            ...
            if row not in self._processed:
                ...
                if not thumbnailPixmap.isNull():
                    ...
                    if thumbnailPixmap.hasAlphaChannel():
                        ...
                        bgBrush.setTexture(QtGui.QPixmap(utils.getMariIconPath('Checker.png')))
                        bgPen = QtGui.QPen()
...
# Viewer Widget
#------------------------------------------------#
class MIBView(QtWidgets.QWidget):
    '''Central Custom Widget for MIBView, handles
    ...
    def setSizeMode(self):
        if cfg.size_mode == 0:
            self.smallIconSizeAction.trigger()
            self.smallIconSizeAction.setChecked(True)
        if cfg.size_mode == 1:
            self.mediumIconSizeAction.trigger()
            self.mediumIconSizeAction.setChecked(True)
        if cfg.size_mode == 2:
            self.largeIconSizeAction.trigger()
    ...
    def setPreset(self, item):
        ...
        if presetType == 'collection':
            if not self.activeCollectionItem:
                ...
                currentPath = self.browserView.fileModel.filePath(self.browserView.fileViewer.rootIndex())
                self.browserView.setDirFromPath(currentPath)
                self.setPathFromBrowser(currentPath)
